...
class mn_TrackTo(Node, AnimationNode):
	...
	def execute(self, input):
		...
		if self.axis[0]:
			X = self.angle(track_y, obj_y, track_z, obj_z,1)
		if self.axis[1]:
			Y = self.angle(track_x, obj_x, track_z, obj_z,-1)
		if self.axis[2]:
			Z = self.angle(track_x, obj_x, track_y, obj_y,1)
